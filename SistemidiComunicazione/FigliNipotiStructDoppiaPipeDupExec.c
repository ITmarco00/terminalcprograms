...
int main(int argc, char **argv) {
	...
	char opzione[5];		/* variabile stringa che serve per passare al comando head l'opzione con il numero delle linee da stampare */
					/* supponiamo che bastino 3 cifre per il numero, oltre al simbolo di opzione '-' e al terminatoredi stringa */
	...
	int finito;			/* variabile che serve al padre per sapere che non ci sono più strutture da leggere */
	
	
	/* Controllo sul numero di parametri: meno di due parametri e non pari */
	if(argc < 3 || (argc-1)%2) {
	...
	/* Controlliamo i nuemri passati: non usiamo ora la funzione atoi() per trasformare queste stringhe in numeri */
	...
	for(i=0; i < N; i++) {
		...
		if(pid == 0) {
			...
			if(pid == 0) {
				...
				/* printf("Sono il processo nipote del figlio di indice %d e pid %d e sto per calcolare la head per il file %s\n", i, getpid(), argv[(i*2)+2]); */
				...
				/* NOTA BENE: non avendo operato l'implementazione della ridirezione dello standard output, ma passando il nome del file come parametro, se per caso il file NON esiste o non fosse leggibile, il comando head fallirebbe e non riuscirebbe a mandare alcuna linea al figlio e quindi il valore 0 tornato dal figlio dovrebbe essere interpretato come problemi appunto nel nipote; in alternativa, per catturare questo problema si potrebbe implementare la ridirezione dello standard input e far terminare con exit(-1) il nipote se la open non va a buon fine, ma in questo caso bisognerebbe che il figlio verificasse il valore di ritorno del nipote e nel caso di -1/255 tornare questo valore al padre! */
				...
				close(p[1]);
				/* adesso il figlio legge dalla pipe fino a che ci sono caratteri cioe' linee inviate dal nipote tramite la head */
				...
				while(read(p[0], &(s.linea_letta[j]), 1)) {
					/* se siamo arrivati all fine di una linea */
					if(s.linea_letta[j] == '\n') {
				...
				if((status & 0xFF) != 0) {
					...
					exit(-1); /* si veda commento precendente */
				}
				else {
	...
	while(!finito) {
		finito = 1; /* mettimao finito a 1 perche' potrebbero essere finiti tutti i figli */
		for(i=0;i<N;i++) {
